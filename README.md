...
1. **Living Matters.** A matter is not a snapshot. Lawyers add material across the life of a case — a follow-up email, a new round of medical records, a codicil that surfaces six weeks late — and the brief should reflect that. Living Matters adds a three-way Add Material affordance, an impact detector that tells the lawyer which sections a new source actually affects, section-level regeneration with diff preview, and a version history that reads like a casebook ledger. The lawyer is in control of every regeneration. See [`docs/LIVING-MATTERS-BRIEF.md`](docs/LIVING-MATTERS-BRIEF.md).
2. **Research Handoff.** A redact-and-export path that lets the lawyer take a redacted draft of the brief plus a list of research questions to a cloud research tool (Claude, ChatGPT, Lexis+ AI, Paxton, Harvey if their firm has it), then bring verified findings back. Docket LM stays on the machine; the paste is the air-gap. Entity pseudonymization, residual-risk surface, encrypted reverse mapping, and a Mata v. Avianca citation-verification gate on re-import. See [`docs/RESEARCH-HANDOFF-BRIEF.md`](docs/RESEARCH-HANDOFF-BRIEF.md).
3. **Bespoke schemas for the remaining four practice areas** (immigration, employment, criminal defense, IP). v1.0 runs these on the general-default eight-section schema; v1.1 adds tuned section vocabularies and per-area retrieval prompts.
4. **v2-moe embeddings.** `nomic-embed-text-v2-moe` (HuggingFace) preserves 768-dim output with a stronger MoE architecture and better legal-domain MTEB scores. Requires a Modelfile shim to register with Ollama. v1.1 swap.
5. **Per-task model routing.** Re-grounding NLI checks don't need 32B parameters. Routing those calls to Qwen3-8B should knock meaningful latency off brief generation at no answer-quality cost — the harness will tell us how much.
6. **Contextual retrieval at ingest.** The eval harness shows we leave retrieval-quality points on the table by skipping per-chunk contextualization. The reason we skip it is ingest time; using the 8B model for the contextualization pass even when the brief uses 32B for generation is a clever workaround.
7. **Multi-matter outlier learning.** The clustering pass that flags "items that don't belong" is single-matter today. Over time, a solo's "what's normal for me" baseline drifts — the system should adapt. Done carefully (without leaking facts across matters), this is a meaningful product expansion.
8. **PACER plugin** (opt-in, the one allowed outbound call). For litigators who pull dockets daily, an in-app PACER fetch beats save-to-folder. Documented explicitly as the one network call Docket makes, and only with the lawyer's per-matter consent.
9. **Windows port** (v1.2). The Tauri shell ports cleanly; the per-format mail and iMessage readers need Windows equivalents (Outlook PST, no direct iMessage analog).
